...
export class AppointmentTrackComponent implements OnInit {
  ...
  async onFileChoose(event: Event, i) {
    this.loading = true;
    let actualExam = this.appointmentForm.get("exams").controls[i].value;
    ...
    this.appointmentForm.get("exams").controls[i].get('url').setValue(imgUrl);
  ...
  async removeExam(i: number) {
    this.loading = true;
    let url = this.appointmentForm.get("exams").controls[i].get('url').value;
